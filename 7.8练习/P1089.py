...
现在请你根据2004年1月到12月每个月津津的预算，判断会不会出现这种情况。如果不会，计算到2004年年末，妈妈将津津平常存的钱加上20％还给津津之后，
津津手中会有多少钱。
输入格式
...
remainmoney = 0
for i in range(12):
    remainmoney = remainmoney + money - usemoney[i]
    if remainmoney < 0 :
        m=i+1
        print(-m)
        exit()
    ...
    bankmoney += savemoney
    remainmoney = remainmoney - savemoney   
print(int(bankmoney*1.2+remainmoney))